function out = bin2DByGroup_equalN(data, varargin)
%BIN2DBYGROUP_EQUALN
% General grouped 2D trajectory binning for paired X/Y data vectors.
%         Version 1.0
%         Date: July 9, 2026
%         Author: G. Herrera 
...
%
% This function divides EACH REPLICATE into N sequential bins with
% approximately equal numbers of data points per bin. It does NOT use
% fixed-width X-value bins.
%
% Example:
%   b = bin2DByGroup_equalN(data, ...
%       'xPath', 'volume', ...
...
%   'nBins'           number of equal trajectory bins; default = 100
%   'minNPerBin'      minimum raw samples/bin/replicate; default = 1
...
%
% Notes:
%   - Group-level SDs are calculated across replicate-level bin means.
%   - This prevents longer recordings from dominating the group mean.
%   - plotBinned2DByGroup can be used directly with this output.
...
p = inputParser;
p.FunctionName = 'bin2DByGroup_equalN';
...
p.addParameter('nBins', 100, @(x) isscalar(x) && isnumeric(x) && x >= 1);
...
opts.nBins = round(opts.nBins);

[xPath, xLabel, xIsPercent] = resolveVar(opts.xVar, opts.xPath, 'x');
...
emptyRep = struct('xMean',nan(nBins,1), ...
                  'xSd',nan(nBins,1), ...
                  'yMean',nan(nBins,1), ...
                  'ySd',nan(nBins,1), ...
                  'n',zeros(nBins,1));
...
for i = 1:nRep
    ...
    if numel(x) ~= numel(y)
        error('bin2DByGroup_equalN:SizeMismatch', ...
    ...
    b = binOneXY_equalN(x, y, nBins, opts.minNPerBin);
...
groups = repmat(struct( ...
    ...
    'yMean',nan(nBins,1), ...
    'ySd',nan(nBins,1), ...
    'nRepsUsed',zeros(nBins,1), ...
    'nRawTotal',zeros(nBins,1)), numel(uGroups), 1);
...
for g = 1:numel(uGroups)
    ...
    groups(g).nRawTotal = sum(nMat, 2);
end

out.binIndex = binIndex;
out.rep = rep;
out.meta = meta;
out.groups = groups;
out.xLabel = xLabel;
...
function b = binOneXY_equalN(x, y, nBins, minN)
...
if nPoints == 0
    b.xMean = xMean; b.xSd = xSd; b.yMean = yMean; b.ySd = ySd; b.n = n;
    return
end

edges = round(linspace(1, nPoints + 1, nBins + 1));
...
function [pathStr, label, isPercent] = resolveVar(varName, pathStr, axisName)
...
if ~isempty(varName)
    ...
    switch key
        ...
        otherwise
            error('bin2DByGroup_equalN:UnknownVarAlias', ...
                'Unknown %sVar alias "%s". Use xPath/yPath for custom fields.', axisName, varName);
    end

elseif ~isempty(pathStr)
    [label, isPercent] = labelFromPath(pathStr);

else
    error('bin2DByGroup_equalN:MissingVariable', ...
        'You must provide either %sVar or %sPath.', axisName, axisName);
end
end

function [label, isPercent] = labelFromPath(pathStr)
switch char(pathStr)
    case 'volume'
        label = 'Volume (mL)';
        isPercent = false;
...
for p = 1:numel(parts)
    ...
    if ~isstruct(val) || ~isfield(val, f)
        error('bin2DByGroup_equalN:MissingField', ...
